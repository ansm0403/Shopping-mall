...
function BannerContent({ bannerImg, children }: BannerContentProps) {
  return (
    <div
      className="flex-shrink-0 w-full h-full flex items-center justify-center text-white text-2xl font-bold bg-cover bg-center bg-no-repeat"
  ...
  return (
    ...
    >
      <div className="grid grid-cols-[2fr_1fr] grid-rows-[1fr_1.5fr] gap-4 py-8 min-h-[500px]">
        ...
        <div
          className="col-start-1 row-span-2 rounded-lg overflow-hidden relative bg-[linear-gradient(135deg,#d2f9a0_0%,#dffd5b_100%)]"
        ...
        >
          ...
          >
            {mainBanners.map((mainBanner) => (
              <BannerContent bannerImg={mainBanner} key={mainBanner} />
            ))}
          </div>
          ...
          <button
            ...
            className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 disabled:opacity-0 text-white w-9 h-9 rounded-full flex items-center justify-center transition-colors z-10"
            aria-label="이전 슬라이드"
          >
            ‹
          </button>
          ...
          <button
            onClick={goToNext}
            disabled={isTransitioning}
            className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-black/60 disabled:opacity-0 text-white w-9 h-9 rounded-full flex items-center justify-center transition-colors z-10"
            aria-label="다음 슬라이드"
          >
            ›
          </button>
